...
export interface Activity5PersonMatchingProps {
...
export const Activity5PersonMatching: React.FC<Activity5PersonMatchingProps> = ({
  activity,
  accent = 'US',
  speechRate = 1.0,
  onNext,
  ...
  const [placements, setPlacements] = useState<Record<string, 'rachel' | 'sara' | null>>({});
  ...
  const handlePlaceIntoCategory = (person: 'rachel' | 'sara') => {
    if (isSubmitted || !selectedItemId) return;
    playFeedbackSound('click');
    setPlacements((prev) => ({
      ...prev,
      [selectedItemId]: person,
    }));
  ...
  const handleDrop = (e: React.DragEvent, person: 'rachel' | 'sara') => {
    e.preventDefault();
    if (isSubmitted) return;
  ...
  const unplacedCount = ACTIVITY_5_ITEMS.filter((item) => !placements[item.id]).length;
  const allCorrect = ACTIVITY_5_ITEMS.every(
    (item) => placements[item.id] === item.correctPerson
  );
  ...
  const rachelItems = ACTIVITY_5_ITEMS.filter((item) => placements[item.id] === 'rachel');
  const saraItems = ACTIVITY_5_ITEMS.filter((item) => placements[item.id] === 'sara');
  const poolItems = ACTIVITY_5_ITEMS.filter((item) => !placements[item.id]);
    ...
    <div className="w-full flex flex-col gap-5">
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 w-full flex flex-col gap-4">
          {/* Table: Rachel vs Sara */}
          <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Column 1: Rachel */}
            <div
              onDragOver={handleDragOver}
              onDrop={(e) => handleDrop(e, 'rachel')}
              onClick={() => handlePlaceIntoCategory('rachel')}
            ...
            >
              <div className="pb-2 border-b border-inherit/40 font-bold text-sm text-sky-600 dark:text-sky-400 flex flex-col">
                <span>Rachel</span>
            ...
            <div
              onDragOver={handleDragOver}
              onDrop={(e) => handleDrop(e, 'sara')}
              onClick={() => handlePlaceIntoCategory('sara')}
              className={`min-h-[220px] rounded-2xl border-2 p-3.5 flex flex-col gap-2 transition-all ${
                selectedItemId
                  ? 'border-dashed border-sky-400 cursor-pointer bg-sky-50/20'
                  : isDark
            ...
            >
              <div className="pb-2 border-b border-inherit/40 font-bold text-sm text-amber-600 dark:text-amber-400 flex flex-col">
                <span>Sara</span>
                <span className="text-xs font-normal text-slate-500 italic">
              ...
              <div className="flex-1 flex flex-col gap-2">
                ...
                {saraItems.map((item) => (
                  <ReversibleOrderItem
                    ...
                    isFlipped={flippedItemIds.includes(item.id)}
                    isSubmitted={isSubmitted}
                    isCorrect={item.correctPerson === 'sara'}
                    onFlip={(e) => toggleItemFlip(item.id, e)}
                    onPlay={(e) => handlePlayText(item.text, item.id, e)}
                    onRemove={(e) => handleRemoveFromCategory(item.id, e)}
                    isDark={isDark}
          ...
          {poolItems.length > 0 && !isSubmitted && (
            ...
            >
              <div className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                Alimentos y bebidas disponibles ({poolItems.length})
              </div>
          ...
          {isSubmitted && (
            ...
            >
              <div className="flex items-center gap-2 font-bold text-sm">
                ...
                <span>{allCorrect ? '¡Todo correcto!' : 'Algunas asociaciones son incorrectas'}</span>
              </div>
...
// Sub-component for placed order items
interface ReversibleOrderItemProps {
  item: PersonOrderItem;
  isFlipped: boolean;
  isSubmitted: boolean;
  isCorrect: boolean;
...
const ReversibleOrderItem: React.FC<ReversibleOrderItemProps> = ({
  item,
  isFlipped,
  isSubmitted,
  isCorrect,
  ...
  return (
    <div className="w-full perspective-1000">
      ...
      >
        {/* Front: English */}
        <div className="absolute inset-0 px-3 py-2 flex items-center justify-between backface-hidden">
          <span className="text-xs sm:text-sm font-medium pr-1 text-slate-900 dark:text-slate-100 capitalize">
            {item.text}
          </span>
        ...
        {/* Back: Spanish */}
        <div className="absolute inset-0 px-3 py-2 flex items-center justify-between backface-hidden rotate-y-180">
          <span className="text-xs sm:text-sm font-medium italic text-slate-700 dark:text-slate-200 capitalize">
            {item.textEs}
          </span>
        </div>
      </div>
