...
export const CleanHouseActivity3: React.FC<CleanHouseActivity3Props> = ({
  ...
  const handleCheck = () => {
    setIsSubmitted(true);
    let allOk = true;
    ACTIVITY_3_DATA.dialogue.forEach((turn) => {
  ...
  return (
    <div className="w-full flex flex-col gap-6 animate-in fade-in duration-300">
      ...
      <div className="w-full perspective-1000">
        ...
        >
          <div className="absolute inset-0 px-5 py-3 flex items-center justify-between backface-hidden">
            <span className="font-semibold text-sm sm:text-base">{ACTIVITY_3_DATA.instructionsEn}</span>
            <button
              type="button"
              onClick={(e) => onPlayAudio(ACTIVITY_3_DATA.instructionsEn, 'act3-instr', e)}
              className={`p-1.5 rounded-lg border transition-all shrink-0 cursor-pointer ${
                playingSentenceId === 'act3-instr'
          ...
          <div className="absolute inset-0 px-5 py-3 flex items-center justify-between backface-hidden rotate-y-180">
            <span className="font-semibold text-sm sm:text-base italic text-slate-700 dark:text-slate-200">
              {ACTIVITY_3_DATA.instructionsEs}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        {/* Right Column: Dialogue & Word Bank */}
        <div className="lg:col-span-8 w-full flex flex-col gap-5">
              ...
              {ACTIVITY_3_DATA.bank.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl border text-xs sm:text-sm font-medium transition-all cursor-pointer ${
                      isUsed
          ...
          <div className="space-y-4">
            {ACTIVITY_3_DATA.dialogue.map((turn) => {
                  ...
                  >
                    ...
                    <div className="w-full p-4 sm:p-5 flex items-start justify-between gap-3 backface-hidden">
                      <div className="flex-1 text-sm sm:text-base leading-relaxed">
                        <span
                          className={`font-bold text-xs px-2 py-0.5 rounded-md mr-2 uppercase tracking-wider ${
                            turn.speaker === 'MARY'
                      ...
                      <button
                        type="button"
                        onClick={(e) => onPlayAudio(turn.fullEn, `act3-${turn.id}`, e)}
                        className={`p-1.5 rounded-lg border transition-all shrink-0 cursor-pointer ${
                          playingSentenceId === `act3-${turn.id}`
                            ? 'bg-sky-500 text-white border-sky-400 ring-2 ring-sky-300'
                            : isDark
                            ? 'bg-slate-800 hover:bg-slate-700 text-sky-300 border-slate-700'
                            : 'bg-sky-50 hover:bg-sky-100 text-sky-600 border-sky-200'
                    ...
                    <div className="absolute inset-0 w-full h-full p-4 sm:p-5 flex items-start justify-between gap-3 backface-hidden rotate-y-180">
                      ...
                      <button
                        type="button"
                        onClick={(e) => onPlayAudio(turn.fullEn, `act3-back-${turn.id}`, e)}
                        className={`p-1.5 rounded-lg border transition-all shrink-0 cursor-pointer ${
                          playingSentenceId === `act3-back-${turn.id}`
                            ? 'bg-sky-500 text-white border-sky-400 ring-2 ring-sky-300'
                            : isDark
                            ? 'bg-slate-800 hover:bg-slate-700 text-sky-300 border-slate-700'
                            : 'bg-sky-50 hover:bg-sky-100 text-sky-600 border-sky-200'
