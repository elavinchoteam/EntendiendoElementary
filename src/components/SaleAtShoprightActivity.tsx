...
import { Activity1ShoprightReading } from './shopright/Activity1ShoprightReading';
import { Activity2FoodVocabulary } from './shopright/Activity2FoodVocabulary';
import { Activity3FoodPriceTable } from './shopright/Activity3FoodPriceTable';
import { Activity4TrueFalseTable } from './shopright/Activity4TrueFalseTable';
import { Activity5ClozeSentences } from './shopright/Activity5ClozeSentences';
import { Activity6ReadingQuestions } from './shopright/Activity6ReadingQuestions';
import { Activity7ShoppingListWriting } from './shopright/Activity7ShoppingListWriting';
import { Activity8ShoprightTests } from './shopright/Activity8ShoprightTests';
import { stopSpeaking } from '../utils/audio';

export interface SaleAtShoprightActivityProps {
  accent?: 'US' | 'UK';
  speechRate?: number;
  onSuccess?: () => void;
}
...
const ACTIVITIES_META = [
  { id: 1, title: 'Actividad 1', label: 'Lectura' },
  { id: 2, title: 'Actividad 2', label: 'Vocabulario' },
  { id: 3, title: 'Actividad 3', label: 'Precios' },
  { id: 4, title: 'Actividad 4', label: 'Verdadero o Falso' },
  { id: 5, title: 'Actividad 5', label: 'Completar Frases' },
  { id: 6, title: 'Actividad 6', label: 'Comprensión' },
  { id: 7, title: 'Actividad 7', label: 'Escritura' },
  { id: 8, title: 'Actividad 8', label: 'Test' },
];

export const SaleAtShoprightActivity: React.FC<SaleAtShoprightActivityProps> = ({
  accent = 'US',
  speechRate = 1.0,
  onSuccess,
}) => {
  ...
  // Current activity index: 0 to 7 (Actividad 1 to Actividad 8)
  ...
  return (
    <div
      id="sale-at-shopright-container"
    ...
    >
      ...
      <button
        type="button"
        id="shopright-floating-prev-btn"
      ...
      <button
        type="button"
        id="shopright-floating-next-btn"
        onClick={() => goToSlide(currentActivityIdx + 1)}
      ...
      <div className="w-full max-w-6xl mx-auto flex flex-col gap-5">
        ...
        <header
          className={`p-3.5 sm:p-4 rounded-2xl border transition-colors shadow-sm flex flex-col md:flex-row items-center justify-between gap-3 ${
        ...
        >
          ...
          <div className="flex items-center gap-2 w-full md:w-auto justify-between md:justify-start">
            <button
              type="button"
              id="shopright-prev-activity-btn"
              onClick={() => goToSlide(currentActivityIdx - 1)}
              disabled={currentActivityIdx === 0}
            ...
            <button
              type="button"
              id="shopright-next-activity-btn"
              onClick={() => goToSlide(currentActivityIdx + 1)}
        ...
        {/* Activity Step Pills (Actividad 1 -> Actividad 8) */}
          ...
          <div className="flex items-center gap-1.5 min-w-max">
            {ACTIVITIES_META.map((act, idx) => {
              ...
              return (
                ...
                >
                  <span>{act.title}</span>
                  {idx === 7 && <Award className="w-3.5 h-3.5" />}
                </button>
              );
            })}
          </div>
