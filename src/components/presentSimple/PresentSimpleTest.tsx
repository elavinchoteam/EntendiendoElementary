...
import {
  PRESENT_SIMPLE_TEST_QUESTIONS,
  PresentSimpleTestQuestion,
...
export interface PresentSimpleTestProps {
  speechRate: number;
  accent: 'US' | 'UK';
  onSuccess?: () => void;
}

export const PresentSimpleTest: React.FC<PresentSimpleTestProps> = ({
  speechRate,
  accent,
  onSuccess,
}) => {
  ...
  const questions: PresentSimpleTestQuestion[] = PRESENT_SIMPLE_TEST_QUESTIONS;
  const currentQ: PresentSimpleTestQuestion = questions[activeTestSubIndex];
  ...
  // 1. Initial Presentation / Start Screen (exact Clean-House Agency format)
  if (!isTestStarted) {
    return (
          ...
          <div className="w-full p-8 sm:p-10 flex flex-col items-center text-center backface-hidden space-y-6">
            ...
            <div className="space-y-2">
              <h3 className="text-2xl sm:text-3xl font-black tracking-tight">
                Present Simple: Statements · Mastery Test
              </h3>
              <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 max-w-lg leading-relaxed">
                Test your knowledge with 5 comprehensive assessment questions covering Present Simple affirmative and negative statements.
              </p>
            </div>
          ...
          <div className="absolute inset-0 w-full h-full p-8 sm:p-10 flex flex-col items-center justify-between text-center backface-hidden rotate-y-180 space-y-6">
            ...
            <div className="space-y-2">
              <h3 className="text-2xl sm:text-3xl font-black tracking-tight text-emerald-600 dark:text-emerald-400 italic">
                Presente Simple: Afirmaciones · Test de Dominio
              </h3>
              <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 max-w-lg leading-relaxed italic">
                Pon a prueba tus conocimientos con 5 preguntas de evaluación integral sobre oraciones afirmativas y negativas en presente simple.
              </p>
            </div>
  ...
  // 2. Completed Test Summary Screen (exact Clean-House Agency format)
  if (isTestCompleted) {
      ...
      <div className="w-full max-w-xl mx-auto my-8 p-8 rounded-2xl border shadow-xl flex flex-col items-center text-center space-y-6 animate-in fade-in duration-300 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700">
        ...
        <div className="space-y-2">
          ...
          <p className="text-slate-600 dark:text-slate-300 text-sm sm:text-base">
            Has completado los 5 tests de la sección Present Simple: Statements.
          </p>
        </div>
  ...
  // 3. Active Test Question Screen (exact Clean-House Agency format)
  const selectedOptId = testUserAnswers[currentQ.id];
  ...
  return (
    <div className="w-full flex flex-col gap-6 animate-in fade-in duration-300">
      ...
      <div className="w-full perspective-1000">
        ...
        >
          <div className="w-full px-5 py-3.5 flex items-center justify-between backface-hidden">
            <span className="font-semibold text-sm sm:text-base">{currentQ.instructionsEn}</span>
            <button
              type="button"
              onClick={(e) => handlePlayAudio(currentQ.instructionsEn, `test-instr-${currentQ.id}`, e)}
          ...
          <div className="absolute inset-0 px-5 py-3.5 flex items-center justify-between backface-hidden rotate-y-180">
            ...
            <button
              type="button"
              onClick={(e) => handlePlayAudio(currentQ.instructionsEn, `test-instr-back-${currentQ.id}`, e)}
              className={`p-1.5 rounded-lg border transition-all shrink-0 cursor-pointer ${
                playingSentenceId === `test-instr-back-${currentQ.id}`
                  ? 'bg-sky-500 text-white border-sky-400 ring-2 ring-sky-300'
                  : isDark
                  ? 'bg-slate-800 hover:bg-slate-700 text-sky-300 border-slate-700'
                  : 'bg-sky-50 hover:bg-sky-100 text-sky-600 border-sky-200'
      ...
      {/* Main Grid: Reference Media Player Left, Question & Options Right */}
      <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 w-full flex flex-col gap-5">
          {/* Question Card (Reversible 3D Card) */}
          <div className="w-full perspective-1000">
            <div
              onClick={toggleQuestionFlip}
              className={`relative w-full min-h-[70px] rounded-2xl cursor-pointer select-none transition-transform duration-500 transform-style-3d border shadow-xs ${
            ...
            >
              ...
              <div className="w-full p-4 flex items-start justify-between backface-hidden">
                <div className="text-base sm:text-lg font-bold flex items-start gap-2.5 flex-1">
                  <span className="w-7 h-7 rounded-full bg-sky-600 text-white text-xs font-mono flex items-center justify-center shrink-0 mt-0.5">
                    {activeTestSubIndex + 1}
                  </span>
                  <div className="whitespace-pre-line leading-relaxed">{currentQ.question}</div>
                </div>
                <button
                  type="button"
                  onClick={(e) => handlePlayAudio(currentQ.audioText, `test-q-${currentQ.id}`, e)}
              ...
              <div className="absolute inset-0 w-full h-full p-4 flex items-start justify-between backface-hidden rotate-y-180">
                <div className="text-base sm:text-lg font-bold italic text-slate-700 dark:text-slate-200 flex items-start gap-2.5 flex-1">
                  <span className="w-7 h-7 rounded-full bg-emerald-600 text-white text-xs font-mono flex items-center justify-center shrink-0 not-italic mt-0.5">
                    {activeTestSubIndex + 1}
                  </span>
                  <div className="whitespace-pre-line leading-relaxed">{currentQ.questionEs}</div>
                </div>
                <button
                  type="button"
                  onClick={(e) => handlePlayAudio(currentQ.audioText, `test-q-back-${currentQ.id}`, e)}
                  className={`p-1.5 rounded-lg border transition-all shrink-0 ml-2 cursor-pointer ${
          ...
          {/* Radio Options (Each option is a Reversible 3D Card with Radio indicator + Speaker button) */}
                ...
                <div key={opt.id} className="w-full perspective-1000">
                  ...
                  >
                    ...
                    <div className="w-full p-3.5 flex items-center justify-between backface-hidden">
                      ...
                      <button
                        type="button"
                        onClick={(e) => handlePlayAudio(opt.text, `test-opt-${opt.id}`, e)}
                        className={`p-1.5 rounded-lg border transition-all shrink-0 ml-2 cursor-pointer ${
                          isDark
                            ? 'bg-slate-800 hover:bg-slate-700 text-sky-300 border-slate-700'
                            : 'bg-sky-50 hover:bg-sky-100 text-sky-600 border-sky-200'
                    ...
                    <div className="absolute inset-0 px-4 py-3 flex items-center justify-between backface-hidden rotate-y-180">
                      ...
                      <button
                        type="button"
                        onClick={(e) => handlePlayAudio(opt.text, `test-opt-back-${opt.id}`, e)}
                        className={`p-1.5 rounded-lg border transition-all shrink-0 ml-2 cursor-pointer ${
                          isDark
                            ? 'bg-slate-800 hover:bg-slate-700 text-sky-300 border-slate-700'
                            : 'bg-sky-50 hover:bg-sky-100 text-sky-600 border-sky-200'
