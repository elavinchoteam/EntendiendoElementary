...
import {
  COMPARATIVES_ACTIVITIES,
  COMPARATIVES_TESTS,
} from '../../data/comparisonComparativesData';
import { ComparativesActivity1Explore } from './ComparativesActivity1Explore';
import { ComparativesDragDropActivity } from './ComparativesDragDropActivity';
import { ComparativesTestActivity } from './ComparativesTestActivity';

export interface ComparisonAdjectivesComparativesSectionProps {
  accent?: 'US' | 'UK';
  speechRate?: number;
  onSuccess?: () => void;
}
...
export const ComparisonAdjectivesComparativesSection: React.FC<ComparisonAdjectivesComparativesSectionProps> = ({
  accent = 'US',
  speechRate = 1.0,
  onSuccess,
}) => {
  ...
  return (
    <div className="relative w-full flex flex-col items-center">
      ...
      <button
        id="comparatives-floating-prev-btn"
        type="button"
        onClick={handlePrev}
      ...
      <button
        id="comparatives-floating-next-btn"
        type="button"
        onClick={handleNext}
            ...
            {isRateMenuOpen && (
              ...
              >
                ...
                {SPEED_OPTIONS.map((opt) => (
                  ...
                  >
                    <span>{opt.label}</span>
                    {currentRate === opt.value && <Check className="w-3.5 h-3.5 ml-1" />}
                  </button>
                ))}
              </div>
            )}
      ...
      <div className="w-full">
        ...
        {currentActivityIdx >= 1 && currentActivityIdx <= 10 && (
          <div className="animate-in fade-in duration-200" key={currentActivityIdx}>
            <ComparativesDragDropActivity
              activity={COMPARATIVES_ACTIVITIES[currentActivityIdx - 1]}
              accent={accent}
              speechRate={currentRate}
        ...
        {currentActivityIdx === 11 && (
          <div className="animate-in fade-in duration-200">
            <ComparativesTestActivity
              tests={COMPARATIVES_TESTS}
              accent={accent}
              speechRate={currentRate}
