...
export interface SuperlativesReversibleCardProps {
  id?: string;
  textEn: string;
  textEs: string;
  speechRate?: number;
...
export const SuperlativesReversibleCard: React.FC<SuperlativesReversibleCardProps> = ({
  id,
  textEn,
  textEs,
  speechRate = 1.0,
    ...
    return parts.map((part, i) => {
      ...
      if (isMatch) {
        return (
          <span
            key={i}
            className="inline-block bg-amber-300 dark:bg-amber-500/30 text-amber-950 dark:text-amber-200 px-1.5 py-0.5 rounded font-bold border border-amber-400/40 shadow-xs"
          >
            {part}
          </span>
        );
